'use strict';
app.directive('promiseBtn', ['$parse','$timeout', function ($parse,$timeout)
{
    'use strict';
                    ...
                    if (mVal && mVal.then) {
                        ...
                        mVal.finally(function ()
                        {
                            if (attrs.delayTime == undefined) {
                                ...
                                handleLoadingFinishedDelay(btnEl);
                            }
...
///////////////// 给倒计时添加的
            function handleLoadingFinishedDelay(btnEl) {

                var time = attrs.delayTime;
                var btnVal = btnEl.text();
                var btnDelayVal = attrs.delayText;

                btnEl.text((btnDelayVal || btnVal) + ' ' + time + 'S');
                timeDown(btnEl, btnDelayVal, btnVal, time);

                if (cfg.btnLoadingClass) {
                    btnEl.removeClass(cfg.btnLoadingClass);
                }
                if (cfg.disableBtn) {
                    $timeout(function() {btnEl.removeAttr('disabled')}, time * 1000);
                }
            }

            function timeDown(btnEl, btnDelayVal, btnVal, num) {
                $timeout(function() {
                    if (num > 1) {
                        num--;
                        btnEl.text((btnDelayVal || btnVal) + ' ' + num + 'S');
                        timeDown(btnEl, btnDelayVal, btnVal, num);
                    } else {
                        btnEl.text(btnVal);
                    }
                }, 1000);
